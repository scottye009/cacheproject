...
//number of cache entries (2^10)
#define L1_NUM_CACHE_ENTRIES (1<<10)
		     ...
		     uint8_t control, uint32_t *read_data, uint8_t *status)
{
  ...
  uint32_t address_index = (address & L1_ADDRESS_INDEX_MASK) >> L1_ADDRESS_INDEX_SHIFT;
  ...
  if (((l1_cache[address_index].v_d_tag & L1_VBIT_MASK) == 0) || ((l1_cache[address_index].v_d_tag & L1_ENTRY_TAG_MASK) != address_tag)){
  ...
  //the entry's dirty bit should be set.
    ...
    if (control & READ_ENABLE_MASK){
      *read_data = l1_cache[address_index].cache_line[word_offset];
    } 

    if (control & WRITE_ENABLE_MASK) {
      l1_cache[address_index].cache_line[word_offset] = write_data;
      l1_cache[address_index].v_d_tag |= L1_DIRTYBIT_MASK;
    }
    *status |= L1_HIT_STATUS_MASK;
  }
}
		    ...
		    uint8_t *status)
{
  ...
  if (((l1_cache[address_index].v_d_tag & L1_ADDRESS_TAG_MASK) != 0 ) && ((l1_cache[address_index].v_d_tag & L1_DIRTYBIT_MASK) != 0)) {
    uint32_t evicted_entry_tag = l1_cache[address_index].v_d_tag & L1_ENTRY_TAG_MASK;
    *evicted_writeback_address = (evicted_entry_tag << L1_ADDRESS_TAG_SHIFT) | (address_index << L1_ADDRESS_INDEX_SHIFT);
    for (int i = 0; i < WORDS_PER_CACHE_LINE; i++) {
      evicted_writeback_data[i] = l1_cache[address_index].cache_line[i];
    }
    *status |= L1_HIT_STATUS_MASK;
  ...
  for (int i = 0; i < WORDS_PER_CACHE_LINE; i++){
    l1_cache[address_index].cache_line[i] = write_data[i];
  }
  
  //set the valid bit, clear the dirty bit, and write the tag
  l1_cache[address_index].v_d_tag = address_tag;
  l1_cache[address_index].v_d_tag |= L1_VBIT_MASK;
  l1_cache[address_index].v_d_tag &= ~L1_DIRTYBIT_MASK;
