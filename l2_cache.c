		     ...
		     uint8_t control, uint32_t read_data[], uint8_t *status)
{
  ...
  uint32_t address_index = (address & L2_ADDRESS_INDEX_MASK) >> L2_ADDRESS_INDEX_SHIFT;
  ...
  int hit = 0;
  int i = 0;
  while (i < L2_LINES_PER_SET){
    //printf("masK:%d\n",L2_VBIT_MASK);
    //printf("resl:%d\n",(l2_cache[address_index].lines[i].v_r_d_tag & L2_ENTRY_TAG_MASK) == address_tag);
    //printf("v%d\n", (l2_cache[address_index].lines[i].v_r_d_tag & L2_VBIT_MASK) && (l2_cache[address_index].lines[i].v_r_d_tag & L2_ENTRY_TAG_MASK) == address_tag);
    if ((l2_cache[address_index].lines[i].v_r_d_tag & L2_VBIT_MASK) && ((l2_cache[address_index].lines[i].v_r_d_tag & L2_ENTRY_TAG_MASK) == address_tag)){
      l2_cache[address_index].lines[i].v_r_d_tag |= L2_RBIT_MASK;
      hit = 1;
      *status |= L2_HIT_STATUS_MASK;

      if (control & READ_ENABLE_MASK){
        for (int j = 0; j < WORDS_PER_CACHE_LINE; j++) {     
          read_data[j] = l2_cache[address_index].lines[i].cache_line[j];
        }
      } 

      if (control & WRITE_ENABLE_MASK) {
        for (int j = 0; j < WORDS_PER_CACHE_LINE; j++){     
          l2_cache[address_index].lines[i].cache_line[j] = write_data[j];
        }
        l2_cache[address_index].lines[i].v_r_d_tag |= L2_DIRTYBIT_MASK;
      }
    }
    i++;
  }
		    ...
		    uint8_t *status)
{
  ...
  for (int i = 0; i < L2_LINES_PER_SET; i++) {
    ...
    if ((l2_cache[address_index].lines[i].v_r_d_tag & L2_VBIT_MASK) == 0) {
      for (int j = 0; j < WORDS_PER_CACHE_LINE; j++) {
        l2_cache[address_index].lines[i].cache_line[j] = write_data[j];
      }
      l2_cache[address_index].lines[i].v_r_d_tag = address_tag;
      l2_cache[address_index].lines[i].v_r_d_tag |= L2_VBIT_MASK;
      l2_cache[address_index].lines[i].v_r_d_tag &= ~L2_DIRTYBIT_MASK;
      l2_cache[address_index].lines[i].v_r_d_tag &= ~L2_RBIT_MASK;
  ...
  for (int i = 0; i < L2_LINES_PER_SET; i++) {
    if (((l2_cache[address_index].lines[i].v_r_d_tag & L2_RBIT_MASK) == 0) && ((l2_cache[address_index].lines[i].v_r_d_tag & L2_DIRTYBIT_MASK) == 0)){
      r0_d0_index = i;
      break;
    }
  }
  ...
  if ((l2_cache[address_index].lines[chosen_index].v_r_d_tag & L2_DIRTYBIT_MASK) != 0){
    uint32_t evicted_entry_tag = l2_cache[address_index].lines[chosen_index].v_r_d_tag & L2_ENTRY_TAG_MASK;
    *evicted_writeback_address = (evicted_entry_tag << L2_ADDRESS_TAG_SHIFT) | (address_index << L2_ADDRESS_INDEX_SHIFT);
    for (int i = 0; i < WORDS_PER_CACHE_LINE; i++) {
      evicted_writeback_data[i] = l2_cache[address_index].lines[chosen_index].cache_line[i];
    }
  ...
  for (int j = 0; j < WORDS_PER_CACHE_LINE; j++) {
    l2_cache[address_index].lines[chosen_index].cache_line[j] = write_data[j];
  }
  l2_cache[address_index].lines[chosen_index].v_r_d_tag = address_tag;
  l2_cache[address_index].lines[chosen_index].v_r_d_tag |= L2_VBIT_MASK;
  l2_cache[address_index].lines[chosen_index].v_r_d_tag &= ~L2_DIRTYBIT_MASK;
  l2_cache[address_index].lines[chosen_index].v_r_d_tag &= ~L2_RBIT_MASK;
