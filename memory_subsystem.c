...
void memory_handle_l1_miss(uint32_t address)  
{
  ...
  l2_cache_access(address, NULL, READ_ENABLE_MASK, read_data, &status); //l2_cache_access(uint32_t address, uint32_t write_data[], uint8_t control, uint32_t read_data[], uint8_t *status)
  ...
  //uint8_t evicted_writeback_status;
